...
describe('useRecipients', () => {
  ...
  it('初回に1ページ目を読み込み、次があればhasMoreを立てる', async () => {
    ...
    expect(mockedFetchFriends).toHaveBeenCalledWith(null);
  });
  ...
  it('loadMoreで次ページを追記し、最終ページでhasMoreがfalseになる', async () => {
    ...
    expect(mockedFetchFriends).toHaveBeenLastCalledWith('C1');
  });
  ...
  it('reloadで1ページ目から取り直す', async () => {
    ...
    expect(mockedFetchFriends).toHaveBeenLastCalledWith(null);
  });
